...
5. **Settings → Check-ins → Send a test check-in** (or the bell icon on the Check-ins card) → a popup appears top-right. Answer it. Expected: toast "Noted: …" and a new line in the *Check-ins* card.
...
Pick **Solo** for now. Right after the profile is created a short **tour** dims the app and points at one control at a time (Start a task, the timer, the activity list, the report, then the Reports, Tasks, Projects and Settings entries) with Next, Back and Skip; it runs once per profile and **Settings → Show the tour** replays it. From then on DailyBee opens on a **lock screen** asking for that password; tracking keeps running behind it. **Settings → Account → Lock now** (or *Lock DailyBee* in the search palette) locks it on demand, and **Change password** and **Solo or Team…** (switch the open profile between the two) sit on the same card. *Forgot your password?* on the lock screen asks your two security questions (answers are not case-sensitive; five wrong tries block it for 30 seconds) and then lets you set a new password. A profile made before security questions existed has none; add them under **Settings → Account → Security questions → Set up**, or its password cannot be reset.
...
- **Search** (magnifier in the top bar, or Ctrl+K): type a task name, an entry, or "report" and press Enter. Actions at the top start or stop a task, generate a report, or send a test check-in.
- **Bell**: a log of what happened in this profile, kept for 30 days: task starts and stops, timer pauses, check-ins, reports and sync trouble. The number on the bell counts unread items; a row jumps to its screen, **Mark all read** and **Clear** do what they say. With **Settings → Notifications → Desktop notifications** on (the default), every one of them also shows as a Windows notification, whether the window is open, hidden in the tray or not open at all; clicking it opens DailyBee.
- **Tasks → New task** creates a real backlog item (it appears in the Start-task list) with a size and a priority (Low, Normal, High, Urgent). Filter the list by project, status, priority and size, or search; **Clear filters** resets them. In **Board** view drag a card to another column to change its status. A Solo profile shows no owners (every task is yours); a team account shows the owner avatar, the **Mine** filter and an owner list of you plus whoever already owns a task.
- **Reports → Week**: the seven days with tracked time, entries, focus and report state (click a day for its breakdown), hours per project against last week and the weekly budget, and the tasks that are overdue or in progress but untouched this week. Arrows move between weeks; **This week** comes back.
- **Without the window**: right-click the tray icon. Idle: **Resume “…”** starts the last task again as it was, **Start recent** lists the tasks of the last two weeks, **Start a task…** opens the dialog. Running: **Stop now** saves the entry as partly done with no questions, **Stop with wrap-up…** opens the dialog. The same toggle sits on a global shortcut, **Ctrl+Alt+D** by default (Settings → Keyboard shortcut, where it can be changed or turned off; the card says whether Windows accepted it). A system notification says which task started or stopped and how much was saved.
- **Reports → History → a row** opens that day with the same cards as Today; **Preview** shows the saved draft without rebuilding it.
- **Correct your history**: on the Entries card (Today, or a day from History) every row has **Correct**, **Split** and **Delete**, and the card has **Add entry** for something the timer missed. A corrected row gets an *Edited* badge (*Added by hand*, *Split off* for the others), the card header counts them, and the History table shows *n edited* per day. The clock icon opens the **Change log**: one line per change, with a hash chained to the line before it; the app can add lines but never alter or remove them, and the dialog says *Chain verified*. The day's report is rebuilt after each correction; one that was already sent shows *Changed since sent* and offers **Send again**.
- **Time away**: with a task running, step away (or lock the screen) for a few minutes and come back. A *Welcome back* card asks whether to **Leave it out**, **Count it as work** or **Stop at hh:mm** (the moment you left); it floats at the top-right of the screen when another app is in front. Settings → Tracking → **Ask about time away** turns it off.
- **What is being recorded**: the line under the task on Today. **Pause tracking** (also in the tray menu) stops the app and tab capture while the timer keeps counting. Settings → Tracking → **Private apps** lists apps that are never recorded; today's captures of a newly added app are removed.
- **Startup**: Settings → Startup → **Launch at login** and **Start in the tray**. They apply to the installed app (the built zip); in a development build the card says so.
- **Updates**: Settings → Updates shows the version and what the self-updater is doing; **Check for updates** asks the feed now. In the installed app a downloaded version shows *Restart to update* here, in the tray and in the bell, and installs when DailyBee quits; after the restart a *What's new* dialog lists the changes once. A development build says so instead of checking.
- **Backup**: Settings → Backup → **Export backup…** saves the whole profile as one `.dailybee` file; **Restore…** brings one back into this profile (the current data is kept beside it). On the profile list, **Restore from a backup…** adds it as a new profile. Solo profiles are reminded in the bell after a month without a backup.
- **Profile picture**: Settings → Profile → **Choose picture…**: pick a JPG, PNG, WebP or GIF, drag it inside the circle and zoom with the slider, **Save picture**. It shows in the sidebar, on the lock screen, in the profile list and on your own row in Team, Admin and Tasks; **Remove** goes back to initials. The picture stays on this device.
- **Reduce animations**: Settings → Appearance. On by default when Windows has *Animation effects* off (Settings → Accessibility → Visual effects); switch it either way, or **Use the system setting** to follow Windows again.
...
- `profiles.json` — the profile list and which profile is open; `profiles\<id>.sqlite` — one database per profile made from the list (name, password hash or workspace token, tracked days, tasks, reports, settings, the change log). An install from before profiles keeps its `dailybee.sqlite` as the *default* profile. Removing a profile from the list deletes its file; a restore keeps the replaced file as `<id>.sqlite.before-restore`.
- Backups (`*.dailybee`) are copies of a profile's database, wherever you saved them. Keep one somewhere other than this PC.
