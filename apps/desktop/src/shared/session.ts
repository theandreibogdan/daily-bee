...
export const PAUSE_LABEL: Record<PauseReason, string> = { idle: 'idle', lock: 'screen locked', sleep: 'asleep', offline: 'app closed' };

/**
 * Active seconds of the current run: banked stretches plus the live one. Time while idle, with the
 * screen locked, the machine asleep or the app closed is not counted (see main/services/session.ts).
 */
