...
export function createMockApi(): DailyBeeApi {
  ...
  const activityRows: ActivityRow[] = [
    ...
    { key: 'slack', app: 'Slack', icon: 'message-square', detail: '#eng-daily, DM Jack', cat: 'communication', seconds: 420, tabs: null, tracked: true },
  ...
  let settings: Settings = {
    profile: { name: 'Mia Lewis', email: 'mia@dailybee.dev', initials: 'ML', role: 'Lead engineer', timezone: 'Europe/Stockholm', avatar: '' },
  ...
  const makeReport = (): ReportDraft => {
    ...
    const r: ReportDraft = { day, label: dayLabel(day), summary: { tracked: total, focus: 71, done: shipped.length, total: entries.length, checkins: 2, distraction: 6 }, shipped, inProgress, mix: [62, 12, 7, 13, 6], narrative: 'Mostly VS Code and GitHub. 9 min on youtube.com at 10:33 — you said “taking a break”.', blockers: 'Staging DB credentials — waiting on Rui, expected tomorrow.', notes: report?.notes ?? 'Blocked on staging DB credentials until Rose is back tomorrow.', markdown: '', status: report?.status ?? 'draft', sentAt: report?.sentAt ?? null, recipients: '#eng-daily · 4 teammates', topApps: ['VS Code', 'GitHub'] };
    r.markdown = `# Daily report — ${r.label}\n\n## Shipped\n${shipped.map((e) => '- ' + e.task).join('\n')}\n\n## In progress\n${inProgress.map((e) => '- ' + e.task).join('\n')}\n\n## Blockers\n${r.blockers}`;
    return r;
  };

  const FAKE_TEAM: TeamData = { members: [
    { initials: 'ML', name: 'Mia Lewis', today: 19860, week: 121500, report: 'Draft', tracking: true }, { initials: 'JK', name: 'Jack King', today: 25200, week: 134400, report: 'Sent', tracking: true }, { initials: 'SO', name: 'Sam Oliver', today: 14400, week: 108000, report: 'Sent', tracking: false }, { initials: 'RA', name: 'Rose Adams', today: 0, week: 96300, report: 'Missing', tracking: false }, { initials: 'TN', name: 'Tom Nash', today: 21600, week: 127800, report: 'Sent', tracking: true }, { initials: 'PB', name: 'Paul Brown', today: 9000, week: 88200, report: 'Draft', tracking: false },
  ...
  const FAKE_ADMIN: AdminData = { orgs: ['Engineering', 'Platform', 'Product'], kpis: { focus: 71, tracked: 178.5, reports: 92, distraction: 6 }, deltas: { focus: '+3 pts', tracked: '+6%', reports: '−4 pts', distraction: '−1 pt' }, categoryMix: { work: 62, research: 12, learning: 7, communication: 13, distraction: 6 },
    people: [
      { initials: 'ML', name: 'Mia Lewis', team: 'Platform', week: 33.75, focus: 78, distraction: 4, reports: 5, mix: [68, 12, 5, 11, 4], top: 'VS Code · GitHub' }, { initials: 'JK', name: 'Jack King', team: 'Engineering', week: 37.3, focus: 74, distraction: 5, reports: 5, mix: [61, 14, 8, 12, 5], top: 'VS Code · Linear' }, { initials: 'SO', name: 'Sam Oliver', team: 'Product', week: 30.0, focus: 58, distraction: 9, reports: 4, mix: [45, 18, 6, 22, 9], top: 'Figma · Slack' }, { initials: 'RA', name: 'Rose Adams', team: 'Platform', week: 26.75, focus: 66, distraction: 3, reports: 2, mix: [70, 10, 4, 13, 3], top: 'Terminal · Grafana' }, { initials: 'TN', name: 'Tom Nash', team: 'Engineering', week: 35.5, focus: 80, distraction: 7, reports: 5, mix: [66, 9, 9, 9, 7], top: 'IntelliJ · GitHub' }, { initials: 'PB', name: 'Paul Brown', team: 'Product', week: 24.5, focus: 52, distraction: 12, reports: 3, mix: [40, 20, 10, 18, 12], top: 'Notion · Zoom' },
    ],
    ...
    alerts: [['danger', 'Rose Adams has not sent a report for 3 days'], ['warning', 'web-app is 1h over its weekly budget'], ['warning', 'Paul Brown: distraction share 12% (team avg 6%)'], ['info', '3 tasks re-assessed from Medium to Large this week']],
